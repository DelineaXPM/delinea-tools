...
func streamLaunch(command []string, env []string, payload []byte) error {
	...
	// A nil Env makes exec.Cmd inherit the entire parent environment; the
	// exec-replace path instead hands nil to syscall.Exec as an empty
	// environment, and childEnv's contract is that only declared variables
	// reach the child. A non-nil empty slice keeps the two paths identical.
	if env == nil {
		env = []string{}
	}
	cmd.Env = env
