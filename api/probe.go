...
// health endpoint is tried first, then the Platform one. It sends no
// credential and ignores every credential field on cfg, so a probe can never
...
func ProbeBackend(ctx context.Context, cfg Config) (Backend, error) {
	...
	for _, p := range probes {
		ok, err := probeHealthy(ctx, client, base+"/"+p.path)
		switch {
		case ok:
...
func probeHealthy(ctx context.Context, client *http.Client, url string) (bool, error) {
