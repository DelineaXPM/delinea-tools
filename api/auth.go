...
func (c *Client) grantOnce(ctx context.Context, endpoint string, form url.Values) (grantResponse, int, string, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		// A transient status is transport-class, not an authentication
		// answer: labeling it ErrAuth once made the secrets resolver report
		// a rate-limited token endpoint as access denied. A 3xx (the grant
		// never follows redirects) means the URL points at a front door that
		// bounces the grant elsewhere — a permanent misconfiguration.
		kind := ErrAuth
		switch {
		...
		return grantResponse{}, resp.StatusCode, resp.Header.Get("Retry-After"),
			fmt.Errorf("%w: token endpoint returned %s: %s", kind, c.authSnippet([]byte(resp.Status)), c.authSnippet(body))
	}
	if oversized {
