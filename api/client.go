...
type Config struct {
	...
	// carry metadata only: never a credential, a request or response body, or
	// a URL query string.
...
// surfacing as a transport error. The replay is not gated to idempotent
// methods the way send's transport retries are: a 401 is a completed response
// the server rejected before acting, so re-sending a mutating request after
// re-auth cannot double-apply it. A 403 is left as-is — an authorization
...
// validateHTTPHeaders applies net/http's wire-level header rules before any
