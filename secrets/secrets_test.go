...
func TestResolveValidatesWholeBatchBeforeFetcher(t *testing.T) {
	f := newFake(map[int]*Secret{9: {Fields: []SecretField{field("password", "p")}}})
	...
	if err == nil {
		t.Fatal("got nil error, want the malformed second mapping rejected")
	}
	if len(f.calls) != 0 {
		t.Errorf("a valid mapping was fetched before the malformed batch entry was found: %v", f.calls)
	}
}
